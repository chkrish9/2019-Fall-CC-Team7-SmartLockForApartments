...
router.get("/get/:name", config.checkToken, (req, res, next) => {
  var name = req.params.name;
  //console.log(name);
  User.getUserNames(name, (err, data) => {
    data.dateOfJoin = new Date(data.dateOfJoin).toLocaleDateString("en");
    res.json(data);
  });
});

router.get("/getid/:id", config.checkToken, (req, res, next) => {
      ...
      Room.getRoomById(req.body.roomId, (err, room) => {
        if (err) {
          ...
          Room.updateRoom(room._id, room, (err, room) => {
            ...
            email.sendEmail(res, user.email, subject, emailBody, {
              success: true,
              id: user._id,
              msg: "User Added."
            });
